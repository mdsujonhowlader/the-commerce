...
export const getCustomerAddress = asyncHandler(async (req, res) => {
    ...
    const address = (foundUser.addresses || []);
    const items = [...address]
...
export const updateCustomerAddressById = asyncHandler(async (req, res) => {
    const dbUser = await getDbUserFromReq(req);
...
export const deleteAddressById = asyncHandler(async (req, res) => {
    ...
    const foundUser = requireFound(user, "User not Found on DB");
    const addresses = (foundUser.addresses || []);
    const adressesUserWantToDeleted = addresses.findIndex((currentAddress) => String(currentAddress._id) === addressId);
